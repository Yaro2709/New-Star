                    ...
                    <div class="content_box">
                        ...
                        {elseif $Element.buyable}
                            <form action="game.php?page=formgovernment" method="post" class="build_form">
                                ...
                                <button type="submit" class="btn_build">{$LNG.fg_one} {foreach $Element.costResources as $RessID => $RessAmount} {$LNG.tech.{$RessID}} <span style="color:{if $Element.costOverflow[$RessID] == 0}lime{else}red{/if}">{$RessAmount|number}</span>{/foreach}</button>
                            </form>
                        {else}
                            <span class="btn_build red">{$LNG.fg_one} {foreach $Element.costResources as $RessID => $RessAmount} {$LNG.tech.{$RessID}} <span style="color:{if $Element.costOverflow[$RessID] == 0}lime{else}#666666{/if}">{$RessAmount|number}{/foreach}</span></span>
                        {/if}
                        </div>
                    </div>
                </div>
