                ...
                <div id="ofic_{$ID}" class="build_box valueTest2">
                    ...
                    <div class="content_box">
                        ...
                        {if $Element.maxLevel <= $Element.level}
                            <span class="btn_build red valueTest6">{$LNG.et_yes}</span>
                        {elseif $Element.buyable}
                            <form action="game.php?page=ethics" method="post" class="build_form">
                                <input type="hidden" name="id" value="{$ID}">
                                <button type="submit" class="btn_build valueTest2">{$LNG.et_one} {foreach $Element.costResources as $RessID => $RessAmount} {$LNG.tech.{$RessID}} <span style="color:{if $Element.costOverflow[$RessID] == 0}lime{else}red{/if}">{$RessAmount|number}</span>{/foreach}</button>
                            </form>
                        {else}
                            <span class="btn_build red valueTest6">{$LNG.et_one} {foreach $Element.costResources as $RessID => $RessAmount} {$LNG.tech.{$RessID}} <span style="color:{if $Element.costOverflow[$RessID] == 0}lime{else}#666666{/if}">{$RessAmount|number}{/foreach}</span></span>
                        {/if}
                        </div>
                    </div>
                </div>
