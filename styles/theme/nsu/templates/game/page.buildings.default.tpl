                    ...
                    {foreach $BuildInfoList as $ID => $Element}
                        <div id="build_{$ID}" class="build_box {if $ID == in_array($ID, $reslist.spec_build.1)}build1{elseif $ID == in_array($ID, $reslist.spec_build.2)}build2{elseif $ID == in_array($ID, $reslist.spec_build.3)}build3{elseif $ID == in_array($ID, $reslist.spec_build.4)}build4{/if} {if !$Element.techacc}required{/if} valueTest2 height164">
                            ...
                            <div class="content_box">
                                <div class="image valueTest2">
                                    <a href="#" onclick="return Dialog.info({$ID})"><img src="{$dpath}gebaeude/{$ID}.gif" alt="" style="border-radius: 2px;"/></a>
                                </div>
                                {if !$Element.techacc}
                                <div class="prices">
                                ...
                                <div class="prices">
                                    ...
                                    <div class="price res{$RessID} {if $Element.costOverflow[$RessID] == 0}{else}required{/if}">
                                        <div class="ico"></div>
                                        <div class="text">{$RessAmount|shortly_number}</div>
                                    </div>
                                    {/foreach}    
                                </div>  
                                ...
                                <div class="time_build">
                                    ...
                                    <span class="time_build_text valueTest2"><img src="{$dpath}img/iconav/time.png" style="height: 11px; top: -1px; position: relative;"> <span class="time_build_edit">{$Element.elementTime|time}</span></span>
                                    {/if}
                                </div>
                                ...
                                <div class="btn_build_border">
                                    <span class="btn_build red valueTest6">{$LNG.bd_working}</span>
                                </div>
                                {else}
                                {if $RoomIsOk}
                                ...
                                <div class="btn_build_border">
                                    <form class="build_form" method="post" action="game.php?page=buildings">
                                        ...
                                        <input id="b_input_{$ID}" class="build_number valueTest" type="number" value="{$Element.levelToBuild + 1}" min="{$Element.levelToBuild + 1}" maxlength="3" size="3" name="lvlup" onchange="counting('{$ID}');"></input>
                                        <button class="btn_build_part_left valueTest2" type="submit">{$LNG.bd_build_next_level}</button>
                                    </form>
                                </div>
                                {else}
                                <div class="btn_build_border">
                                    <span class="btn_build red valueTest6"> {if $Element.level == 0}{$LNG.bd_build_next_level}{else}{$LNG.bd_build_next_level}{$Element.levelToBuild + 1}{/if}</span>
                                </div>
                                {/if}
                                {else}
                                <div class="btn_build_border">
                                    <span class="btn_build red valueTest6">{$LNG.bd_no_more_fields}</span>
                                </div>
                                {/if}
                                {/if}    
                            </div>
