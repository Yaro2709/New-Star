...
{if isset($Info)}	
    ...
    <div style="float:left; width:378px;" class="batle_party">
        <table>
            <tbody><tr>
                <td style="text-align: right;padding-right: 7px;font-size: 22px;font-weight: bold;padding: 25px 0 25px;color:{if $Raport.result == "a"}lime{elseif $Raport.result == "r"}red{else}white{/if}">{$Info.0}</td>
    ...
    <div style="float:right" class="batle_party">
        <table>
            <tbody><tr>                            	
                ...
                <td style="text-align: left;font-size: 22px;font-weight: bold;padding: 25px 0 25px;color:{if $Raport.result == "r"}lime{elseif $Raport.result == "a"}red{else}white{/if}">{$Info.1}</td>
            </tr>
        </tbody></table>
    </div>                    
    <div id="batle_vs">
  ...
  {/foreach}      		        
    ...
    <div class="batle_part_def">
        ...
        <div class="batle_mem {if $RoundInfo@last}{if $Raport.result == "r"}batle_mem batle_mem_die{else}batle_mem{/if}{/if}">
            <div class="batle_mem_header">
            	<span>{$PlayerInfo.name}
			            {if !empty($Player.DK) && $Player.DK > 1}<span class="tooltip" data-tooltip-content="Умение академии" style="color:#FC0; cursor:help;">[Точные выстрелы]</span>{/if}
                        {if !empty($Player.SK) && $Player.SK > 1}<span class="tooltip" data-tooltip-content="Умение академии" style="color:#39F; cursor:help;">[Двойные щиты]</span>{/if}
						</span> 
                {if isset($Info)}([XX:XX:XX]){else}([{$PlayerInfo.koords[0]}:{$PlayerInfo.koords[1]}:{$PlayerInfo.koords[2]}]{if isset($PlayerInfo.koords[3])}{/if}){/if}<br>
				          {$LNG.sys_ship_weapon} <span>+{$PlayerInfo.tech[0]}%</span> •
                	{$LNG.sys_ship_shield} <span>+{$PlayerInfo.tech[1]}%</span> • 
...
{$LNG.fleet_defs_1} {$RoundInfo.info[2]|number} {$LNG.fleet_defs_2} {$RoundInfo.info[1]|number} {$LNG.damage}">
   ...
   <div class="left_part_att" style="width:{({$RoundInfo.info[2]|number} / ({$RoundInfo.info[0]|number} + {$RoundInfo.info[2]|number}))*100}%">
        <div class="bsorb" style="width:{({$RoundInfo.info[1]|number} + 1 / ({$RoundInfo.info[2]|number} + 1))*100}%">
            <div class="bsorb_end"></div>
            <div class="bsorb_mid"></div>
        </div>    </div>
    <div class="left_part_def" style="width:{({$RoundInfo.info[0]|number} / ({$RoundInfo.info[0]|number} + {$RoundInfo.info[2]|number}))*100}%">
        <div class="bsorb" style="width:{({$RoundInfo.info[3]|number} + 1 / ({$RoundInfo.info[0]|number} + 1))*100}%">
            <div class="bsorb_end"></div>
            <div class="bsorb_mid"></div>
        </div>    </div>
</div>
...
{$LNG.sys_defender_lostunits} {$Raport['units'][1]|number} {$LNG.sys_units}
    ">
        ...
        <div class="left_part_att" style="width:{({$Raport['units'][0]|number} / ({$Raport['units'][1]|number} + 0.1 + {$Raport['units'][0]|number}))*100}%">
        </div>
        <div class="left_part_def" style="width:{({$Raport['units'][1]|number} / ({$Raport['units'][1]|number} + 0.1 + {$Raport['units'][0]|number}))*100}%">
        </div>
    </div>
